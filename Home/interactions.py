...
from dynamic_functions.Home.common import _read_json, _write_json, require_game_dir, require_membership, _safe_id
...
@visible
async def interaction_list(game_key: str, bot_sid: str) -> List[Dict[str, Any]]:
    """List all recorded interactions for a bot in this game."""
    require_membership(game_key)
    d = _interactions_dir(game_key, bot_sid)
    ...
    for fname in sorted(os.listdir(d)):
        if not fname.endswith(".json"):
            continue
        record = _read_json(os.path.join(d, fname), None)
        if record:
            records.append(record)
    await atlantis.client_data(f"Interactions for {bot_sid}", records)
    return records
