...
A *bot* is an AI character that can fill a role — Kitty, Lars, Sven,
Natalie, Taffy, Chad. Bots are reusable assets: the same bot may drive many
roles. A bot carries its image and LLM config (provider/model/key); prompt
material for a role-specific bot lives in Game/Slots/<role>_<bot>/.
"""
...
async def bot_list() -> List[Dict[str, str]]:
    """List bots (AI characters available to fill roles)"""
    bots = _bot_rows()
    await atlantis.client_data("Bots", bots)
    return bots
